...
function GooFlow(bgDiv, property) {
	...
	//sxf
	GooFlow.prototype.innerWidth = property.innerWidth;
	GooFlow.prototype.innerHeight = property.innerHeight;
	GooFlow.prototype.color = property.colors;
	GooFlow.prototype.haveTool = property.haveTool;
	...
	if (property.haveHead) {
		...
		this.onBtnSaveClick = property.headBtnEvents && property.headBtnEvents.save ? property.headBtnEvents.save : null;//保存流程图按钮定义
		this.onFreshClick = property.headBtnEvents && property.headBtnEvents.reload ? property.headBtnEvents.reload : null;;//重载流程图按钮定义
		if (property.headBtns)
			this.$head.on("click", { inthis: this }, function (e) {
				...
				switch ($(tar).attr("class")) {
					...
					case "ico_moveleft": This.moveAllNode('left'); break;
					case "ico_moveup": This.moveAllNode('up'); break;
					case "ico_moveright": This.moveAllNode('right'); break;
					case "ico_movedown": This.moveAllNode('down'); break;
				}
			});
	}
	var toolWidth = 0;
	...
	if (property.haveTool) {
		...
		this.$workArea = $(`<div class='GooFlow_work_inner' style='width:${property.innerWidth}px;height:${property.innerHeight}px;transform: scale(${property.scaleRuleObj.widthScale},${property.scaleRuleObj.heightScale});'></div>`)
			.attr({ "unselectable": "on", "onselectstart": 'return false', "onselect": 'document.selection.empty()' });
	}
	...
	if (this.$editable) {
		...
		//sxf 加入无向实线
		this.$lineOper = $("<div class='GooFlow_line_oper' style='display:none'><i class='b_l1' title='竖折线'></i><i class='b_l2' title='横折线'></i><i class='b_l3' title='直线'></i><i class='b_x fa fa-trash-o' title='删除'></i><br/><i class='b_l4 fa fa-location-arrow' title='有无箭头'></i><i class='b_l5 fa fa-ellipsis-h' title='是否虚线'></i></div>");//选定线时显示的操作框
		...
		this.$lineOper.on("click", { inthis: this }, function (e) {
			...
			let targClassName = $(e.target).attr("class");
			switch (true) {
				case targClassName.indexOf("b_x") !== -1:	//删除
					This.delLine(id);
					this.style.display = "none"; break;
				case targClassName.indexOf("b_l1") !== -1: //竖折线
					This.setLineType(id, "lr"); break;
				case targClassName.indexOf("b_l2") !== -1: //横折线
					This.setLineType(id, "tb"); break;
				case targClassName.indexOf("b_l3") !== -1: //直线
					This.setLineType(id, "sl"); break;
				case targClassName.indexOf("b_l4") !== -1: //箭头
					This.setLineAttribute(id, "arrow"); break;
				case targClassName.indexOf("b_l5") !== -1: //虚线
					This.setLineAttribute(id, "dashed"); break;
			}
		});
...
GooFlow.prototype = {
	...
	initDraw: function (id, width, height) {
		...
		this.$draw.id = id;
		if (GooFlow.prototype.haveTool) {
			this.$draw.style.width = width * 3 + "px";
			this.$draw.style.height = +height * 3 + "px";
		} else {
			this.$draw.style.width = GooFlow.prototype.innerWidth + "px";
			this.$draw.style.height = +GooFlow.prototype.innerHeight + "px";
		}
		//绑定连线的点击选中以及双击编辑事件
		var tmpClk = null;
		...
		$(this.$draw).delegate(tmpClk, "dblclick", { inthis: this }, function (e) {
			...
			if (GooFlow.prototype.useSVG != "") {
				...
				oldTxt = This.$lineData[this.id].name;
			...
			This.$textArea.val(oldTxt).css({
				display: "block", width: 120, height: 34,
	...
	initGroup: function (width, height) {
		if (GooFlow.prototype.haveTool) {
			...
			this.$group = $("<div class='GooFlow_work_group' style='width:" + GooFlow.prototype.innerWidth + "px;height:" + GooFlow.prototype.innerHeight + "px'></div>");//存放背景区域的容器
		}
	...
	addNode: function (id, json) {
		...
		if (json.type.indexOf(" lcircle") > 0) {
			//如果为链接圆点
			json.width = 2;
			json.height = 2;
			this.$nodeDom[id] = $(`<div class='GooFlow_item lcircle${mark}' id='${id}' style='top:${json.top}px;left:${json.left}px'><table cellspacing='0'><tr><td class='ico'><i class='ico_${json.type}'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'></div></div>`)
		} else if (json.type.indexOf(" round") < 0) {
			//如果为方块
			let className = "GooFlow_item ";
			if (json.type.indexOf(" lrect") > 0) {
				className += "lrect";
				if (!json.width) json.width = this.ConstantData.nodelWidth;
				if (!json.height) json.height = this.ConstantData.nodelHeight;
			} else {
				if (!json.width) json.width = this.ConstantData.nodeWidth;
				if (!json.height) json.height = this.ConstantData.nodeHeight;
			}
			...
			//sxf 判断是否编辑状态显示不同的块
			if (this.haveTool) {
				this.$nodeDom[id] = $(`<div class='${className}${mark}' id='${id}' style='top:${json.top}px;left:${json.left}px'><table cellspacing='1' style='width:${(json.width - 2)}px;height:${json.height - 2}px;'><tr><td class='ico'><i class='fa fa-arrows-alt' aria-hidden='true'></i></td><td class='item-titile'>${json.name}</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>`);
			} else {
				this.$nodeDom[id] = $(`<div class='${className}${mark}' id='${id}' style='top:${json.top}px;left:${json.left}px'><table cellspacing='1' style='width:${(json.width - 2)}px;height:${json.height - 2}px;'><tr><td class='ico'></td><td class='item-titile'>${json.name}</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>`);
			}
			//文本换行
			...
			this.$nodeDom[id].find("td:eq(1)").html(domStr);
	...
	initWorkForNode: function () {
		...
		this.$workArea.delegate(".ico + td", "dblclick", { inthis: this }, function (e) {
			//node节点双击编辑
			// var oldTxt = this.innerText;
			var This = e.data.inthis;
			var id = $(this).parents(".GooFlow_item").attr("id");
			let oldTxt = This.$nodeData[id].name;
			var t = getElCoordinate(This.$workArea[0]);
			This.$textArea.val(oldTxt).css({
				...
				.data("id", This.$focus).focus().select();
			This.$workArea.parent().one("mousedown", function (e) {
				if (e.button == 2) return false;
				This.setName(This.$textArea.data("id"), This.$textArea.val(), "node");
				This.$textArea.val("").removeData("id").hide();
		...
		this.$workArea.delegate(".GooFlow_item > div > div[class!=rs_close]", "mousedown", { inthis: this }, function (e) {
			...
			document.onmousemove = function (e) {
				...
				if (X < GooFlow.prototype.ConstantData.minX) X = GooFlow.prototype.ConstantData.minX;
				if (Y < GooFlow.prototype.ConstantData.minY) Y = GooFlow.prototype.ConstantData.minY;
				isMove = true;
				switch (cursor) {
					case "nw-resize": This.$ghost.css({ width: X - 2 + "px", height: Y - 2 + "px" }); break;
					case "w-resize": This.$ghost.css({ width: X - 2 + "px" }); break;
	...
	//设置结点/连线/分组区域的文字信息
	//sxf:修改了源码，name为^链接的字符串，name^id
	//如果^不存在，则只为name
	setName: function (id, name, type) {
		let myID;
		if (name.indexOf('^') !== -1) {
			let tmpList = name.split('^');
			name = tmpList[0];
			myID = tmpList[1];
			//判断myID的唯一性
			if (this.$nodeData[myID] !== undefined) {
				alert('id已存在');
				return;
			}
		}
		...
		if (type == "node") {//如果是结点
			...
			if (GooFlow.prototype.useSVG != "") {
				...
				let nameList = name.split('\n');
				let domStr = '';
				let originX = Number(this.$lineDom[id].childNodes[2].getAttribute('x'));
				let originY = Number(this.$lineDom[id].childNodes[2].getAttribute('y'));
				for (let i = 0; i < nameList.length; i++) {
					domStr += `<tspan x='${originX}' y='${originY + i * 14}'>${nameList[i]}</tspan>`
				}
				this.$lineDom[id].childNodes[2].innerHTML = domStr;
			}
			else {
		...
		if (myID) {
			if (type == "node") {//如果是结点
				//DOM的ID属性修改
				$(`#${id}`).attr('id', myID);
				this.$nodeDom[myID] = this.$nodeDom[id];
				//删除旧ID
				delete this.$nodeDom[id];
				//新增ID
				this.$nodeData[myID] = this.$nodeData[id];
				//删除旧ID
				...
				for (let lineID in this.$lineData) {
					if (this.$lineData[lineID]['to'] === id) {
						this.$lineData[lineID]['to'] = myID;
					}
					if (this.$lineData[lineID]['from'] === id) {
						this.$lineData[lineID]['from'] = myID;
					}
				}
			} else if (type == "line") {//如果是线
				//DOM的ID属性修改
				$(`#${id}`).attr('id', myID);
				this.$lineDom[myID] = this.$lineDom[id];
				//删除旧ID
				delete this.$lineDom[id];
				//新增ID
				this.$lineData[myID] = this.$lineData[id];
				//删除旧ID
	...
	loadData: function (data) {
		...
		//如果为浏览态则隐藏连接点
		if (GooFlow.prototype.haveTool === false) {
			$('.GooFlow_item.lcircle').css('display', 'none')
		}
	},
	//用AJAX方式，远程读取一组数据
	...
	addLineDom: function (id, lineData) {
		...
		if (lineData.type == "sl")
			this.$lineDom[id] = GooFlow.prototype.drawLine(id, res.start, res.end, lineData.marked, lineData.dashed, lineData.noArrow);
		else
			this.$lineDom[id] = GooFlow.prototype.drawPoly(id, res.start, res.m1, res.m2, res.end, lineData.marked, lineData.dashed, lineData.noArrow);
		...
		if (GooFlow.prototype.useSVG == "") {
			...
			//文本换行
			let nameList = lineData.name.split('\n');
			let domStr = '';
			let originX = Number(this.$lineDom[id].childNodes[2].getAttribute('x'));
			let originY = Number(this.$lineDom[id].childNodes[2].getAttribute('y'));
			for (let i = 0; i < nameList.length; i++) {
	...
	addLine: function (id, json) {
		...
		this.$lineData[id].dashed = json.dashed;
		this.$lineData[id].noArrow = json.noArrow;
		//sxf 初始化json赋值
	...
	resetLines: function (id, node) {
		for (var i in this.$lineData) {
			...
			if (this.$lineData[i].type == "sl") {
				this.$lineDom[i] = GooFlow.prototype.drawLine(i, res.start, res.end, this.$lineData[i].marked, this.$lineData[i].dashed, this.$lineData[i].noArrow);
			}
			else {
				this.$lineDom[i] = GooFlow.prototype.drawPoly(i, res.start, res.m1, res.m2, res.end, this.$lineData[i].marked, this.$lineData[i].dashed, this.$lineData[i].noArrow);
			}
			...
			if (GooFlow.prototype.useSVG == "") {
				...
				//文本换行
				let nameList = this.$lineData[i].name.split('\n');
				let domStr = '';
				let originX = Number(this.$lineDom[i].childNodes[2].getAttribute('x'));
				let originY = Number(this.$lineDom[i].childNodes[2].getAttribute('y'));
				for (let i = 0; i < nameList.length; i++) {
					domStr += `<tspan x='${originX}' y='${originY + i * 14}'>${nameList[i]}</tspan>`
				}
				this.$lineDom[i].childNodes[2].innerHTML = domStr;
			}
		}
	},
	//重新设置连线的样式 attr = 'arrow':修改连接线是否有箭头
	setLineAttribute: function (id, attr) {
		if (attr === 'arrow') {
			//sxf childNodes[1]为路径，childNodes[2]为text文本
			this.$lineData[id].noArrow = !this.$lineData[id].noArrow;
			...
			this.$lineData[id].dashed = !this.$lineData[id].dashed;
			if (this.$lineData[id].dashed) {
				this.$lineDom[id].childNodes[1].setAttribute("style", "stroke-dasharray:6,5");
			} else {
				this.$lineDom[id].childNodes[1].setAttribute("style", "stroke-dasharray:none");
			}
		}
	},
	...
	setLineType: function (id, newType, M) {
		...
		//如果是变回直线
			...
			this.$lineDom[id] = GooFlow.prototype.drawLine(id, res.start, res.end, this.$lineData[id].marked || this.$focus == id, this.$lineData[id].dashed, this.$lineData[id].noArrow);
			this.$draw.appendChild(this.$lineDom[id]);
			if (GooFlow.prototype.useSVG == "") {
				...
				//文本换行
				let nameList = this.$lineData[id].name.split('\n');
				let domStr = '';
				let originX = Number(this.$lineDom[id].childNodes[2].getAttribute('x'));
				let originY = Number(this.$lineDom[id].childNodes[2].getAttribute('y'));
				for (let i = 0; i < nameList.length; i++) {
	...
	setLineM: function (id, M, noStack) {
		...
		this.$draw.removeChild(this.$lineDom[id]);
		this.$lineDom[id] = GooFlow.prototype.drawPoly(id, ps.start, ps.m1, ps.m2, ps.end, this.$lineData[id].marked || this.$focus == id, this.$lineData[id].dashed, this.$lineData[id].noArrow);
		this.$draw.appendChild(this.$lineDom[id]);
		if (GooFlow.prototype.useSVG == "") {
			this.$lineDom[id].childNodes[1].innerHTML = this.$lineData[id].name;
	...
	//移动所有节点元素
	moveAllNode(direction) {
		const unitLen = 50;
		switch (direction) {
			case 'up':
				for (let node in this.$nodeData) {
					this.$nodeData[node].top = this.$nodeData[node].top - unitLen
				}
				break;
			case 'left':
				for (let node in this.$nodeData) {
					this.$nodeData[node].left = this.$nodeData[node].left - unitLen
				}
				break;
			case 'down':
				for (let node in this.$nodeData) {
		...
		let tmpNodeData = { ...this.$nodeData };
		let tmpLineData = { ...this.$lineData };
		this.clearData();
		for (let i in tmpNodeData) {
			this.addNode(i, tmpNodeData[i]);
		}
		for (let j in tmpLineData) {
			this.addLine(j, tmpLineData[j]);
		}
	}
}
